...
export class HeilongjiangSettingComponent extends AppComponentBase implements AfterViewChecked {
  ...
  constructor(injector: Injector, public yibaoservice: HeilongjiangYibaoService,private route:ActivatedRoute,private router:Router) {
    super(injector);
  ...
  getClinicConfigragtion(): void {
    this.yibaoservice.getClinicConfigragtion().subscribe(result => {
      this.reports = [];
  ...
  getAgencyConfiguration(): void {
    this.yibaoservice.getAgencyConfiguration().subscribe(result => {
      this.reports = [];
      for (const item in result) {
        let ideclarationModel: declareReportModel = {
          name: item,
