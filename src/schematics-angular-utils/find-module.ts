...
export function findRouteModuleFromOptions(host: Tree, options: ModuleOptions): Path | undefined {
  if (options.hasOwnProperty('skipImport') && options.skipImport) {
    return undefined;
  }
  ...
  if (!options.module) {
    ...
    return normalize(findRouteModule(host, pathToCheck));
  } else {
    const modulePath = normalize(
      '/' + (options.path) + '/' + options.module);
    const moduleBaseName = normalize(modulePath).split('/').pop();
    ...
    if (host.exists(modulePath)) {
      ...
      throw new Error('Specified route module does not exist');
    }
  }
}

export function findRouteModule(host: Tree, generateDir: string): Path {
  let dir: DirEntry | null = host.getDir('/' + generateDir);
