...
function createAddToModuleContext(host: Tree, options: ModuleOptions): AddToModuleContext {
  ...
  const componentPath = `${options.path}/${stringUtils.dasherize(options.name)}/${stringUtils.dasherize(options.name)}-${stringUtils.dasherize(options.type||'')}.component`;

  result.relativePath = buildRelativePath(options.module, componentPath);
  result.classifiedName = `${stringUtils.classify(options.name)}${stringUtils.classify(options.type||'')}Component`;
